...
= #redact(alter: "roife")[#resume_name] #box(width: 376pt) #box(baseline: 5%,width: 80pt,std.image("1c1.jpg"))
  ...
  == #fa[#briefcase] 工作经历
    ...
    #cventry(
        ...
        - 参与商家组、会员组、库存组的项目开发，包含商家工作台、管理中心、集成系统的功能迭代与优化：
          - 主导融仓智联项目从0到1的开发，现已对接450个ERP系统，进行库存对接的商家达3000+；
          - 参与聚库通、采购管理项目的开发，优化查询慢SQL达20余个，针对300w数据量的库存数据导入由5分钟优化至80s左右；
  ...
  == #fa[#project-diagram] 项目经历
    ...
    #cventry(
        tl: [融仓智联 · *2024年开思金马团队奖*],
        bl: [系统主业务为库存对接，通过外部调用开思标准化接口或主动获取ERP数据实现;],
        br: [Java / SpringCloud],
        tr: [开思],
    )[
    ...
    #cventry(
         tl: [小猫管家公寓管理系统],
         bl: [小猫管家是一个为房东管理公寓提供便捷的系统项目，其主要功能有房源管理、设备管理、租约管理、人员管理以及财务管理。],
         br: [Java / SpringCloud],
         tr: [智能屋],
    )[
        - 负责服务端核心功能的开发和优化，如对接设备数据入库、提高有关设备请求响应速度等；
        - 负责基于RabbitMQ的MQTT移动物联套件，实现设备端与服务端双向通信，设备连接与消息并发。；
        - 负责权限模块中使用Spring Security搭建认证授权服务，支持密码以及验证码登录，通过单点登录时使用JWT生成token，引入Redis存储用户token以及权限信息。
    ]
